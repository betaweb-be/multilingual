...
pimcore.document.tree = Class.create({
    ...
    treeDataUrl: "/plugin/SEInternationalisation/document/tree-get-childs-by-id/",
    ...
    setHeader: function() {
        ...
        // Autoload the first selected language
        //this.loadLanguage();
    ...
    loadLanguage: function() {
        ...
        pimcore.globalmanager.get("layout_document_tree").tree.getLoader().load(pimcore.globalmanager.get("layout_document_tree").tree.root, function() {document.getElementById('documentLanguage').disabled = false;});
    ...
    showAddDocumentDialog: function(title, label, callback, values) {
        // new window
        // define the window.
        //
        var textfields = [];
        var i = 0;
        textfields[i++] = new Ext.form.Label({
            id: 'add_document_label',
            fieldLabel: label,
            width: 275
        });
        ...
        Ext.getCmp('key_' + pimcore.settings.websiteLanguages[0]).on('keyup', function(){
            var val = Ext.getCmp('key_' + pimcore.settings.websiteLanguages[0]).getValue();
            for (var i=1; i<pimcore.settings.websiteLanguages.length; i++) {
                Ext.getCmp('key_' + pimcore.settings.websiteLanguages[i]).setValue(val);
            }
        });
        win.show();
    },

    // TODO: proper oplossen, zodat dit niet dubbel geprogrammeerd hoeft te worden...
    showEditDocumentDialog: function(me, title, label, id, callback) {
        console.log(callback);
        Ext.Ajax.request({
            url: "/plugin/SEInternationalisation/document/get-related-documents",
            params: {
                'language': pimcore.globalmanager.get("layout_document_tree").tree.getLoader().baseParams.language,
                'id': id
            },
            success: function(responseObject) {
                ...
                //me.showAddDocumentDialog(title, label, callback, response.data);		// TODO
...
//        }
        ...
        if (this.attributes.permissions.publish && this.attributes.type != "folder" && !this.attributes.locked && this.parentNode.id != 1) {
            if (this.attributes.published) {
                var unpublishMenu = [];
                unpublishMenu.push({
                    text: t('unpublish_this_page'),
                    iconCls: "pimcore_icon_tree_unpublish",
                    handler: this.attributes.reference.publishDocument.bind(this, this.attributes.type, this.attributes.id, 'unpublish',false)
                });

                unpublishMenu.push({
                    text: t('unpublish_all_pages'),
                    iconCls: "pimcore_icon_tree_unpublish",
                    handler: this.attributes.reference.publishDocument.bind(this, this.attributes.type, this.attributes.id, 'unpublish',true)
                });
                ...
                var publishMenu = [];
                publishMenu.push({
                    text: t('publish_this_page'),
                    iconCls: "pimcore_icon_tree_publish",
                    handler: this.attributes.reference.publishDocument.bind(this, this.attributes.type, this.attributes.id, 'publish',false)
                });
                publishMenu.push({
                    text: t('publish_all_pages'),
                    iconCls: "pimcore_icon_tree_publish",
                    handler: this.attributes.reference.publishDocument.bind(this, this.attributes.type, this.attributes.id, 'publish',true)
                });
...
//        }
    ...
    addDocument : function (type, docTypeId) {
        console.log(this);
        console.log(this.attributes.reference);
        this.attributes.reference.showAddDocumentDialog(t('add_document'), t('please_enter_the_name_of_the_new_document'), this.attributes.reference.addDocuments.bind(this, type, docTypeId));
    },

    publishDocument: function (type, id, task, all) {
        if (all == undefined) {
            all = false;
        }
        var parameters = {};
        ...
        if (all) {
            preUrl = '/plugin/SEInternationalisation/';
        } else {
            preUrl = '/admin/';
        }
        Ext.Ajax.request({
            url: preUrl + type + '/save/task/' + task,
            method: "post",
            params: parameters,
    ...
    addDocuments: function(type, docTypeId, button, event) {
        ...
        data['index'] = this.childNodes.length;
        ...
        Ext.Ajax.request({
            url: "/plugin/SEInternationalisation/document/add",
            params: data,
            success: this.attributes.reference.addDocumentComplete.bind(this)
        });
    ...
    editDocumentKeyComplete: function (response) {
        ...
        try {
            var rdata = Ext.decode(response.responseText);
            if (rdata && rdata.success) {
                if (pimcore.globalmanager.exists("document_" + this.id)) {		// TODO documenten in andere talen ook herladen!!!
    ...
    deleteDocument : function () {
        Ext.Ajax.request({
            url: "/plugin/SEInternationalisation/document/delete-info",
            params: {id: this.id},
...
//        pimcore.helpers.deleteDocument(this.id);
    ...
    deleteDocumenFromServer: function (r) {
        if (r.deletejobs) {
            ...
            for(var i = 0;i<r.deletejobs.length;i++) {
                if (pimcore.globalmanager.exists("document_" + r.deletejobs[i][0].params.id)) {
                    var tabPanel = Ext.getCmp("pimcore_panel_tabs");
                    tabPanel.remove("document_" + r.deletejobs[i][0].params.id);
                }
            }
